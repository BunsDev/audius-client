...
export const fetchSupporting = async (userId: ID, limit = LIMIT) => {
  try {
    ...
    console.error(`Could not fetch supporting for user id ${userId}: ${(e as Error).message}`)
    return []
  }
}
...
export const fetchSupporters = async (userId: ID, limit = LIMIT) => {
  try {
    ...
    console.error(`Could not fetch supporters for user id ${userId}: ${(e as Error).message}`)
    return []
  }
}
